...
Utils.getDateTime = function () {
	return (Date.now() % parseInt("0xffffffffl", 16)).toString(16).toUpperCase();
};
...
Utils.getFormattedCounter = function (counter) {
	counter = counter || Counter.getCounterIncrement();
...
Utils.convertCommandToBuffer = function (command) {
	...
	var lenghtByte = (command.split(',').pop().length / 2).toString(16);
	lenghtByte = lenghtByte.length === 1 ? '0' + lenghtByte : lenghtByte;
	...
	_tmp.forEach(function (item, idx) {
		...
		if (idx === 32) {
			console.log('lengthbyte', lenghtByte, lenghtByte.length, (lenghtByte != '00'));
			if (lenghtByte != '00') {
				_tmp2.push(lenghtByte);
			} else {
				_tmp2.push('0e');
			}
		}
