...
Pairing.prototype.startPairingFlow = function(data) {
	...
	console.log('pairing flow:', data);
};

Pairing.prototype.isValidPairingRequest = function(data) {
	// split up data, we are only in the payload
...
Pairing.prototype._onFirstPairingResponse = function() {
...
Pairing.prototype.sendSecondPairingRequest = function(hmId, owner) {
	// command template
	var cmd = '{{timestamp}},00,00000000,01,{{timestamp}},{{counter}}A001{{owner}}{{hmId}}000802010A{{own1}}0B{{own2}}0C{{own3}}';
